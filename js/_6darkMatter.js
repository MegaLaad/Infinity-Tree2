addLayer("dm", {
    ...
    update() {
        if (getBuyableAmount('dm', 11).gte(1)) player.dm.cationsEffect = Decimal.pow(5, getBuyableAmount('dm', 11))
        else player.dm.cationsEffect = decimalOne

        if (getBuyableAmount('dm', 12).gte(1)) player.dm.cationsNerf = Decimal.pow(getBuyableAmount('dm', 12).add(1), 2)
        ...
        if (getBuyableAmount('dm', 11).gte(1)) player.dm.anionsNerf = Decimal.pow(getBuyableAmount('dm', 11).add(1), 2)
