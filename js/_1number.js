addLayer("n", {
    ...
    update() {
        if (hasUpgrade('n', 11) && (player.points < 1e309 || hasUpgrade('i', 21))) player.points = player.points.times(new Decimal(1).add(Decimal.pow(getBuyableAmount('n', 12).add(2), new Decimal(0.1).times(getBuyableAmount('n', 11).add(1)))).pow(Decimal.min(player.i.points.add(1), 100)).pow(Decimal.min(player.v.points.times(10).add(new Decimal(1).sub(player.v.pointsTest)), 100)))
    },
    ...
    buyables: {
        ...
        12: {
            ...
            display() { return "Increase the Speed Modifier power.<br><br>Cost: "+format(this.cost(getBuyableAmount(this.layer, this.id)))+" number<br>Bought: "+Decimal.floor(format(getBuyableAmount(this.layer, this.id)))+"/10" },
            canAfford() { return player.points.gte(this.cost()) },
            buy() {
                player.points = player.points.div(this.cost())
                setBuyableAmount(this.layer, this.id, getBuyableAmount(this.layer, this.id).add(1))
